...
import AdicionarLoteManual from '../components/AdicionarLoteManual';
    ...
    <div>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
              <button className="btn primary" onClick={() => setShowScanner(true)}
                ...
                <Icon name="barcode" size={14} /> Escanear etiqueta
              </button>
              <button className="btn" onClick={() => setShowManual(true)}
                style={{ flex: '1 1 180px', justifyContent: 'center' }}>
                <Icon name="pencil" size={14} /> Adicionar manual
              </button>
            </div>
            ...
            {linhas.length === 0 && (
              <div style={{ padding: '20px 0', textAlign: 'center', fontSize: 12, color: 'var(--text-3)' }}>
                ...
                <span style={{ fontSize: 11 }}>
                  Escaneie a etiqueta de cada lote ou use "Adicionar manual"
                  quando a etiqueta não estiver disponível.
                </span>
              </div>
            )}
      ...
      {showManual && (
        <AdicionarLoteManual
          ...
          onLoteEncontrado={(lote) => { adicionarLote(lote); setShowManual(false); }}
        />
      )}
    </div>
  );
